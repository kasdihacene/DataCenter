 ...
 * 
 *          The rolling-average is preferably an average of four consecutive
 *          required average amounts pushed by the adaptable entity
 *          <code>RequestDispatcherComponent</code>
 *
 */
public class AdapterRequestDispatcher extends ResourceInspector implements DataPushDispatcherReceiverI {
	...
	public void connectAdapterWithProvider(String providerURI) throws Exception {
		dataProviderOutboundPort.doConnection(providerURI + "_DPIP", DataProviderConnector.class.getCanonicalName());
		dataProviderOutboundPort.doConnection(providerURI + "_DDIP", DataProviderDispatcherConnector.class.getCanonicalName());
	}
	...
	/**
	 * receives the pushes from the Request Dispatcher component and do adaptation
	 * if necessary.
	 */
	@Override
	...
	public void updateCoreFrequency(String applURI, Frequency_gap frequency_gap) throws Exception {
		...
		if (!computerCores.isEmpty()) {
			for (AllocatedCore core : computerCores.keySet()) {
				...
				acop.doConnection(computerInfo.getComputerURI() + "_ACIP",
						AdapterComputerConnector.class.getCanonicalName());
	...
	public void allocateNewAVM(String ApplicationURI) throws Exception {
		...
		System.out.println();
		System.out.println(
				"+++++++++++++++++++++ AVAILABLE COMPUTERS FOR " + getAppURI() + " : " + availableComputers.size());
		...
		if (!availableComputers.isEmpty()) {
			...
			synchronized (dispatcherInfo) {
				...
				aVmAdaptable.doPortConnection(avmURI + "_RNOP", getAppURI() + "RD_RNIP",
	...
	public boolean adapteNewComponent(String computerURI, ApplicationVMAdaptable avm) throws Exception {
		try {
			...
			// Connect the AVM to Request Dispatcher for sending Notifications
			avm.doPortConnection(avmURIrecentlyAdded + "_RNOP", getAppURI() + "RD_RNIP",
					RequestNotificationConnector.class.getCanonicalName());
	...
	public void removeCoreFromAVM() throws Exception {
		...
		if (!"".equals(avmURI)) {
			System.out.println();
			System.err.println(">>>>>>>>>>>>>>>>>>>>>>>> AVM AND COMPUTER USED ==> " + avmURI + "  "
					+ listAVMInformation.get(avmURI).getComputerURI());
			// Connect with the AVM less efficient and add him a Core
			avmiop.doConnection(avmURI + "_AVMIP", AdapterVMConnector.class.getCanonicalName());
			...
			// remove Processor Core concretely on <code>Processor</code>
			acop.doConnection(listAVMInformation.get(avmURI).getComputerURI() + "_ACIP",
					AdapterComputerConnector.class.getCanonicalName());
			acop.releaseCore(core);
			acop.doDisconnection();
		} else {
	 ...
	 * 
	 * @author Hacene KASDI Class to initialize threshold variables read from an
	 *         adjustment file
	 */
	public static class ControllerSetting {
		...
		public static HashMap<String, String> getTuninigs(String file) throws Exception {
			...
			try {
				/*
				 * Désolé mais ca ne marche pas en multijvm.
				 * Créer un scanner est bloquant ...
				 */
				...
				reader.close();
				/*
				tunings.put("averageThreshold", "1000");
				tunings.put("freaquenceCoefficient","1.2");
				tunings.put("coreCoefficient","1.4");
				tunings.put("VMCoefficient","2.5");
				tunings.put("NBCONSECUTIVEAVERAGE","10");
				tunings.put("interavalAdaption","2500");
				*/
			} catch (Exception fnf) {
				...
				System.err.println("Sorry, the system could not start!");
				*/
				System.err.println("Default settings");
				tunings.put("averageThreshold", "1000");
				tunings.put("freaquenceCoefficient","1.2");
				tunings.put("coreCoefficient","1.4");
				tunings.put("VMCoefficient","2.5");

 ...
 *
 */
public class AdmissionController extends ResourceInspector implements AdmissionRequestHandlerI {
	 ...
	 * 
	 * @param riURI
	 *            : Resource Inspector URI
	 * @param acvm
	 * @throws Exception
	 */
	public AdmissionController(String acURI, AbstractCVM acvm) throws Exception {
		super(acURI);
	...
	public void inspectResourcesAndNotifiy(AdmissionI admission) throws Exception {
		...
		if (availableComputerURI != null) {
			...
			System.err.println(
					"Computers available for : " + admission.getApplicationURI() + " == " + availableComputerURI);
			...
			anop.doConnection(admission.getAdmissionNotificationInboundPortURI(),
	...
	/**
	 * Allow reserving resources for the given application and create AVM,
	 * RequestDispatcher
	 * 
	 * @param admissionI
	 * @param computerURI
	...
	protected AdmissionI launchCooperation(AdmissionI admissionI) throws Exception {
		...
		// Create the Adapter Component and launch it
		Coordinator coordinator = new Coordinator(admissionI.getApplicationURI() + "RD",

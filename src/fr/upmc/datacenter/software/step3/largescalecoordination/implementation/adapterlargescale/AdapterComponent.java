package fr.upmc.datacenter.software.step3.largescalecoordination.implementation.adapterlargescale;
 ...
 * @version 02.01.2018
 * 
 * Used on Large scale cooperation
 * This class <code>AdapterComponent</code> represents the Adapter of the current behavior 
 ...
 *
 */
public class AdapterComponent 	extends 	ResourceInspector 
	...
	public AdapterComponent(String riURI, String applicationURI) throws Exception {
		super(riURI);
		this.appURI				= applicationURI;
		requestResponsesInfo	= new LinkedList<fr.upmc.datacenter.software.step2.adapter.InfoRequestResponse>();
	...
	{
	final AdapterComponent AdapterComponent = this ;
	this.pushingFutureTasks =
			this.scheduleTask(
					new ComponentI.ComponentTask() {
						...
						public void run() {
							try {
								Integer size = AdapterComponent.getsizeTasks(avmURI);
								if(size > 0 )
									AdapterComponent.seekForStateTaskAVM(avmURI);
							} catch (Exception e) {
		...
		this.pushingFuture =
				this.scheduleTask(
						new ComponentI.ComponentTask() {
							...
							public void run() {
								try {
									AdapterComponent.launchAdaption();
